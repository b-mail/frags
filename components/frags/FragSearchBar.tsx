...
export default function FragSearchBar() {
  ...
  return (
    <div className="flex w-full min-w-0 flex-col items-center justify-between gap-4 rounded-2xl bg-slate-900 p-4 shadow-2xl md:p-6">
      ...
      <div className="flex w-full flex-col gap-2 md:flex-row md:items-center md:justify-between md:gap-4">
        <button
          className={`${filter === "all" ? "cursor-default bg-slate-800 text-green-400" : "cursor-pointer text-slate-500 hover:bg-slate-800 hover:text-slate-400"} flex-1 rounded-xl py-2 text-center text-sm md:rounded-2xl md:text-base`}
          onClick={() => setFilter("all")}
        >
          전체 FRAGS
        </button>
        <button
          className={`${filter === "member" ? "cursor-default bg-slate-800 text-green-400" : "cursor-pointer text-slate-500 hover:bg-slate-800 hover:text-slate-400"} flex-1 rounded-xl py-2 text-center text-sm md:rounded-2xl md:text-base`}
          onClick={() => setFilter("member")}
        >
          내가 가입한 FRAGS
        </button>
        <button
          className={`${filter === "admin" ? "cursor-default bg-slate-800 text-green-400" : "cursor-pointer text-slate-500 hover:bg-slate-800 hover:text-slate-400"} flex-1 rounded-xl py-2 text-center text-sm md:rounded-2xl md:text-base`}
